...
### 3. Класифікація (для кожного концепту)
...
## 2. Попередня обробка зображень (Предетектори)
...
#### 2.2.1 Морфологічне Скелетонізування
...
#### 2.2.2 Алгоритм Growing Neural Gas
...
### 2.4 Первинний аналіз контуру

#### 2.4.1 Структурні елементи та їх опис
...
### 3.3 Параметри структурних елементів
...
Для переходу від кількісного опису геометрії до якісного, що є ключовим для узагальнення та формування абстрактних концептів, у проєкті вводиться ієрархічна система просторової сегментації. Ця система структурує простір у відносній системі координат (СК_R) і дозволяє класифікувати вектори та точки контуру на основі їхнього розташування відносно початкової точки. Сегментація функціонує як набір порогових рівнів для визначення якісних просторових параметрів.

Процес сегментації реалізовано на трьох рівнях деталізації:

1.  **Сегментація на півплощини:** На найвищому рівні абстракції простір ділиться на дві півплощини (наприклад, верхню та нижню, або ліву та праву) відносно осі, що проходить через початкову точку. Це дозволяє отримати найбільш загальну якісну характеристику напрямку вектора.
...
Цей ієрархічний підхід дозволяє системі аналізувати об'єкти на різних рівнях абстракції. Використовуючи ці сегментації як порогові рівні, система перетворює неперервні кутові метрики на дискретний набір якісних ознак. Наприклад, замість точного кута вектора (наприклад, 45 градусів), система класифікує його як такий, що належить до "першого квадранта" або навіть до певного "сегмента першого квадранта". Такий підхід значно підвищує стійкість моделі до незначних варіацій у формі та орієнтації, що є необхідною умовою для створення узагальнених візуальних концептів.
...
### 3.5 Структура зміни параметрів, сегментація

#### 3.5.1 Енергетичні рівні параметрів
...
#### 3.5.2 Стратегії енергетичної оптимізації за типами даних

##### 3.5.2.1 Абсолютні значення (числові параметри)
...
##### 3.5.2.2 Списки значень (множинні параметри)
...
##### 3.5.2.3 Строкові значення (категоріальні параметри)
...
#### 3.5.3 Приклади енергетичної оптимізації

##### 3.5.3.1 Абсолютні значення
