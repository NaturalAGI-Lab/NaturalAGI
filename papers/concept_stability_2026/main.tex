...
concepts, node and edge counts freeze within tens of samples for twelve of
thirteen concepts while the parameter envelope keeps widening (one
concept's formation crashes), the concepts generalize to uncurated MNIST
splits at reduced accuracy, and concepts store 92.5$\times$ fewer nodes
...
training samples, which derive from 86 unique source images, and every
...
scope and failure counts stated (Section~\ref{sec:studies}).
...
counts after few samples in twelve of thirteen concepts, and compresses
input structure by 92.5$\times$ in nodes and 51$\times$ in stored values
...
formed from 875 pre-augmented training samples. Those 875 samples come from
86 unique source images; the other 789 are geometric variants generated
...
three random draws per condition --- 156 planned runs, of which eight
failed (all of them \cid{4\_1} formations, see below); reported means
average the surviving draws only. Two monotone trends emerge among
successful runs: node counts \emph{decrease or stay constant}
...
Among successful runs, larger sample sets widen the parameter envelopes
...
form at higher sample counts (2 of 3 draws crash at $n{=}5$, all draws at
$n \geq 10$), showing that for topologically heterogeneous classes the
reduction can fail outright rather than degrade gracefully. Measuring
...
\cid{9\_2} & 16 & 10 & 88 \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $^\dagger$Probe crashed at sample 12 of
42; censored as a failure,}\\
\multicolumn{4}{l}{\footnotesize not counted as convergence.}
\end{tabular}
\end{table}

Among the twelve concepts whose formation completed, simple concepts
...
sample-count
study: late samples tune tolerances, not structure. \cid{4\_1} did not
complete: its probe crashed at sample 12 of 42, so it enters
Table~\ref{tab:convergence} as a censored failure, corroborating the
formation fragility seen in Section~\ref{sec:samplecount}.
...
parameters). Node compression is $9{,}800/106 = 92.5\times$ overall
(up to 176.7$\times$ for \cid{2\_1}; counting nodes plus edges gives
92.7$\times$), and stored-value compression is 51$\times$. Values are
...
study), freezes node and edge counts after few samples in twelve of
thirteen concepts (convergence study), and compresses 875
...
approached from different initial conditions. They do not prove one: we
have not defined a convergence test over full attributed graphs, the
order study shows residual structural variation in \cid{2\_2}, and
\cid{4\_1} crashes rather than converging. \cid{4\_1} marks the limit
...
after 1--37 samples in the twelve concepts whose formation completed
(one crashed), with node
compression of 92.5$\times$ and stored-value compression of 51$\times$.
...
projection is the open downstream alternative. Reduction operators need
hardening for
topologically heterogeneous classes, and the augmentation and
