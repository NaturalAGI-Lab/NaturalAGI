...
On the complete-contour test subset --- the input domain the contour
representation is built for --- the system reaches 91.13\% on classified
images, which supports the structural-concept design in its current scope.
Generalization tests bound uncurated-MNIST end-to-end accuracy at 82.21\%
on the test
split and 73.42\% over all 70{,}000 images; the drop is carried by the
...
that predicts the missing contour element and the completed input graph is
reduced again, leaving the concept itself unchanged.
...
snapshots), the analysis notebooks, the offline probe caches, and the
dataset-variant run exports are available in the \texttt{main} branch of
the project
repository\footnote{\url{https://github.com/kbokh/NaturalAGI}}.
