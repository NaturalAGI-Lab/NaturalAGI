...
The graph representation makes structure the carrier of explanations, providing inherent interpretability without post-hoc approximation. Graph cycles directly encode topological properties: closed-loop digits (6, 9) exhibit mean degree 2.0 and contain intersection points, while open-contour digits exhibit lower mean degrees (1.33--1.71) and terminal endpoints (Table~\ref{tab:concept_metrics}). Node degree distributions reveal branching patterns, enabling direct queries such as ``which concepts contain intersection points?'' or ``what is the typical corner count for digit 3?'' Parametric generalization (\autoref{subsec:parametric_generalization}) encodes decision boundaries as explicit attribute ranges rather than implicit weight matrices: normalized coordinate spans (e.g., $x \in [-0.7, 0.2]$ for digit 3) define acceptable variation transparently, while categorical properties persist only when consistent across all training samples.
...
Future work should address these constraints through multiple avenues. Approximate GED algorithms leveraging learned node embeddings \cite{wang2021combinatorial} or direct matching prediction \cite{liu2023mata} could reduce computational overhead while preserving interpretability through explicit edit path recovery. Comparative evaluation of contour extraction techniques (edge detection, active contours) would clarify robustness to preprocessing choices. Rotation normalization via principal component alignment or learned canonical orientations could extend invariance beyond augmentation ranges. Finally, hybrid representations combining contour graphs with texture descriptors or regional features could broaden the approach to natural images and complex visual domains while maintaining structural transparency.
