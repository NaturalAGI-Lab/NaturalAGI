...
class EndpointReductionStrategy(AbstractReductionStrategy):
    """
    This is basically the same as the endpoint reduction strategy in the concept creator,
    ...
    def reduce(
        ...
        if concept_endpoints and not image_endpoints:
            raise ValueError("Concept has endpoints but image does not.")
        ...
        if len_concept_endpoints > len_image_endpoints:
            raise ValueError("Concept has more endpoints than image.")
        else:
    ...
    def _get_endpoints(self, graph: nx.Graph) -> List[Any]:
        return [
            ...
            # TODO: check if this is correct for concept to refine endpoint detection
    ...
    def _find_endpoints_below_threshold(
        ...
        for i, endpoint_id in enumerate(endpoints):
            if max_similarity[i] < self.similarity_threshold:
                self.logger.info(
                    f"Endpoint {endpoint_id} has similarity {max_similarity[i]} below threshold {self.similarity_threshold}. Adding to list of endpoints to remove."
                )
    ...
    def _apply_reduction(self, graph: nx.Graph, endpoints: List[Any]) -> nx.Graph:
        ...
        if all_nodes_to_remove:
            self.logger.info(
                f"Removing {len(all_nodes_to_remove)} nodes from graph. Nodes: {all_nodes_to_remove}"
            )
    ...
    def _find_endpoint_path_to_remove(
        ...
        while queue:
            ...
            is_intersection = (
                graph.degree(current_node) > 2
            )  # This node can not have intersection point label
