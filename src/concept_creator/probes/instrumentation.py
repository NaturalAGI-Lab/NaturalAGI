...
def _make_align_paths_wrapper(recorder: FormationRecorder, original, step_ref: list[int]):
    # Emits one segment_match event per template row from the full alignment.
    def _wrapper(similarity_matrix):
        match = original(similarity_matrix)
        s = step_ref[0]
        used = [j for j in match if j is not None]
        crossing = used != sorted(used)
        many_to_one = len(used) != len(set(used))
        for row_idx, best_idx in enumerate(match):
            scores = list(similarity_matrix[row_idx]) if similarity_matrix else []
...
def attach_instrumentation(service, recorder: FormationRecorder) -> list[int]:
    ...
    # 3. Wrap SyncedGraphMinorFinder._align_paths
    finder = service.graph_minor_finder
    original_align = finder._align_paths
    finder._align_paths = _make_align_paths_wrapper(
