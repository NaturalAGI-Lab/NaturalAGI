...
# Fixed: the clobber line was removed from synced_traversal_generator.py, so the
# accumulated set {0, 1} is preserved. Verify both in-simulation and in-source.
print(f"Visited set preserved: visited[{pair_AX}] = {visited[pair_AX]}")
assert visited[pair_AX] == {path_0, path_1}, "visit records must accumulate"
...
with open(src_path) as f:
    source = f.read()
assert "visited[(next_c_critical_point, next_i_critical_point)] = {path_id}" not in source, \
    "clobber line must stay removed"
print("Source check: clobber line absent from synced_traversal_generator.py")
print("→ FIXED: cycle detection records are no longer overwritten.")
