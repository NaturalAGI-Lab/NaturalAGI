...
class ContourAnalysisRepository:
    ...
    def analyze_contour(self):
        ...
        with self.driver.session() as session:
            ...
            result = session.write_transaction(
                self.process_contour, min_angle_point, self.calculate_and_set_relative_params
            )
            logging.debug(f"Result from calculate_and_set_relative_params: {result}")
            
            result = session.write_transaction(
                self.process_contour, min_angle_point, self.calculate_magnitude_and_direction
            )
    ...
    @staticmethod
    def process_contour(tx, min_angle_point, procedure):
        ...
        while True:
            ...
            procedure(tx, last_vector, next_vector, coords, last_direction, current_angle_point)
    ...
    def get_next_vector_details(
        tx, current_angle_point, processed_vectors, last_vector, min_angle_point
    ):
        if len(processed_vectors):
            # Fetch the details of the current AnglePoint
            vector_details_query = """
                MATCH (vector:Vector)--(ap:AnglePoint)--(nextVector:Vector), (nextVector)--(loc:VectorLocation)--(coords:VectorCoordinates), (ap)--(apLoc:AnglePointLocation)
            ...
            """
            vector_details_result = tx.run(
                vector_details_query, ap_id=current_angle_point["id"]
            )
            # Fetch the details of the current AnglePoint
            vector_details_record = vector_details_result.single()
            vector_details = vector_details_record["VectorDetails"]
            # Find the next line based on coordinates
            return *find_next_vector(vector_details, min_angle_point), current_angle_point

    @staticmethod
    def calculate_and_set_relative_params(tx, last_vector, next_vector, coords, last_direction, current_angle_point):
    ...
    @staticmethod
    def calculate_magnitude_and_direction(tx, last_vector, next_vector, coords, last_direction, current_angle_point):
        ...
        direction_change, current_direction = check_direction_change(
            tx, last_vector, next_vector, last_direction
        )
