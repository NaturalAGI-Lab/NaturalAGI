...
def check_direction_change(tx, image_id, vector1, vector2, last_direction):
    logging.info(
        f"Checking for direction change between vector {vector1['vector_id']} and vector {vector2['vector_id']}"
    )
    query = """
    ...
    record = tx.run(
        query,
        vector1_id=vector1["vector_id"],
        vector2_id=vector2["vector_id"],
        image_id=image_id,
    ).single()
    ...
    if record:
        ...
        logging.info(
            f"Direction change: {direction_change}, Current direction: {current_direction}"
        )
...
def add_direction(tx, vector1, vector2, direction):
    ...
    query = f"""
        ...
        MERGE (vd:VectDirection {{vector1_id: $vector1_id, vector2_id: $vector2_id}})
        MERGE (direction:{direction})
        MERGE (vd)-[:IS]-(direction)
...
def create_critical_point(tx, vector1, vector2):
    ...
    """
    tx.run(
        query, vector1_id=vector1["vector_id"], vector2_id=vector2["vector_id"]
    ).single()
