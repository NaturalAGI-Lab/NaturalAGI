...
1. Редукція точки перетину (`IntersectionPoint`) до кутової точки (`CornerPoint`):
...
Ці підтипи мають набільше значення енергії і, при спрощенні, ця енергія зменшується. Тобто при редукції типу точки (від точки перетину до кінцевої точки чи кутової точки) енергія зменшується.

Також для значення енергії ми маємо різні параметри структурних елементів. Наприклад, для відрізка (`Vector`) ми маємо наступні параметри:

- `x1` та `y1` - координати початку відрізка. Абсолютні значення.
- `x2` та `y2` - координати кінця відрізка. Абсолютні значення.
- `length` - довжина відрізка. Абсолютне значення.
- `angle_with_origin` - кут між відрізком та віссю X. Абсолютне значення (від 0 до 360 градусів з кроком в 5 градусів).
- `quadrant` - квадрант, в якому знаходиться відрізок (при аналізі контуру зі стартової точки). Шкала від 1 до 4.
- `horizontal_direction` - напрямок горизонтального розвитку вектора. Шкала ["RIGHT", "LEFT"].
- `vertical_direction` - напрямок вертикального розвитку вектора. Шкала ["TOP", "BOTTOM"].


Для точкок ми маємо наступні параметри:

- `x` та `y` - координати точки. Абсолютні значення.
- `relative_distance` - відстань до центру структури. Абсолютне значення.
- `angle` - кут між двома відрізками, що сходяться в цій точці. Абсолютне значення (від 0 до 360 градусів).
- `segments` - сегменти, в яких знаходиться ця точка відносно центру структури. Шкала ["TOP", "BOTTOM", "LEFT", "RIGHT"].

Тип структурного елементу також є спеціальним параметром, який визначає тип/підтип структурного елементу.

При навчанні (формуванні концепту) ми узагальнюємо значення параметрів і переходимо на інші енергетичні рівні, тим самим реалізуючи градієнтний спуск по енергетичному ландшафту для пошуку значення параметру, який буде належати до усіх тренувальних данних даного концепту на знайденому енергетичному рівні.

Для абсолютних значень, таких як координати, довжина відрізка, відстань до центру структури, ми мінімізуємо енергію знаходячи діапазон значень, які будуть належати до усіх тренувальних данних даного концепту. 
Тобто ми маємо 2 рівні енергії для абсолютних значень:
1. Вищий рівень енергії - абсолютне значення величини параметра.
2. Нижчий рівень енергії - діапазон значень, який буде належати до усіх тренувальних данних даного концепту.

##### Приклад: 

Структурний елемент першого тренувального семплу має величину кута 90 градусів. 
- Якщо для усіх тренувальних данних величина кута відповідного структурного елементу буде 90 градусів, то ми не повинні робити градієнтний спуск по енергетичному ландшафту і ми залишаємось на вищому рівні енергії зі значенням кута 90 градусів.
- Якщо є тренувальні дані з величиною кута відмінною від 90 градусів, то ми повинні робити градієнтний спуск по енергетичному ландшафту і знаходимо діапазон значень, який буде належати до усіх тренувальних данних даного концепту.
