...
import requests
from critical_points_repository import CriticalPointsRepository

from pydantic_settings import BaseSettings


HANDLER_NAME = "Critical Points Detector"
...
def init_context(context):
    ...
    points_repository = CriticalPointsRepository(Settings().neo4j_dsn, Settings().neo4j_user, Settings().neo4j_pass)
    setattr(context.user_data, "critical_points_repository", points_repository)
...
def http_handler(context, event):
    ...
    try:
        ...
        context.user_data.critical_points_repository.find_and_create_points()
